...
import 'dart:async'; // التأكد من استيراد dart:async
...
final securityStatusProvider = StateProvider<bool>(
    (ref) => SecurityIntegration.instance.isSecurityServiceConnected());
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  void initState() {
    ...
    // إضافة الاشتراك في تحديثات حالة خدمات الأمان
    _subscribeToSecurityStatus();

    // تحقق أولي من حالة خدمات الأمان
    _checkSecurityStatus();

    // تحقق من نزاهة الجهاز في الخلفية
  ...
  void _subscribeToSecurityStatus() {
    _securityStatusSubscription = SecurityIntegration.instance
        ...
        .listen((isConnected) {
      ref.read(securityStatusProvider.notifier).state = isConnected;
    });
  }

  // التحقق الأولي من حالة خدمات الأمان
  void _checkSecurityStatus() {
    final isConnected =
        SecurityIntegration.instance.isSecurityServiceConnected();
    ref.read(securityStatusProvider.notifier).state = isConnected;
  }

  // التحقق من نزاهة الجهاز في الخلفية
  Future<void> _verifyDeviceIntegrity() async {
    try {
      final isDeviceSecure =
          await SecurityIntegration.instance.verifyDeviceIntegrity();

      if (!isDeviceSecure && mounted) {
        // عرض تحذير للمستخدم إذا لم يكن الجهاز آمناً
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'تحذير: تم اكتشاف بيئة تشغيل غير آمنة. بعض الميزات قد تكون محدودة.',
  ...
  Widget build(BuildContext context) {
    ...
    final isSecurityConnected = ref.watch(securityStatusProvider);
        ...
        actions: [
          ...
          Padding(
            ...
            child: Icon(
              ...
              color:
                  isSecurityConnected ? Colors.green[200] : Colors.orange[300],
              size: 20,
            ),
          ),
          IconButton(
      ...
      bottomNavigationBar: BottomNavigationBar(
        items: _navBarItems.map((item) {
          return BottomNavigationBarItem(
            ...
            activeIcon: Icon(item['selectedIcon'] as IconData),
...
class HomePageDialogs {
  ...
  static void showMoreOptionsBottomSheet(
    ...
    final currentTheme = Theme.of(context);
    final isSecurityConnected = ref.read(securityStatusProvider);
    final logger = ref.read(appLoggerProvider);
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // إضافة عنصر لعرض حالة اتصال خدمات الأمان
              _buildBottomSheetTile(
                context: bottomSheetContext,
                icon: isSecurityConnected
                    ? Icons.verified_outlined
                    : Icons.security_update_warning_outlined,
                iconColor: isSecurityConnected
                    ? Colors.green[600]
                    : Colors.orange[700],
                title: isSecurityConnected
                    ? 'خدمات الأمان متصلة'
                    : 'جاري التحقق من خدمات الأمان',
                textColor: isSecurityConnected
                    ? Colors.green[600]
                    : Colors.orange[700],
                onTap: () async {
                  bool isMountedBSCtx = bottomSheetContext.mounted;
                  ...
                  if (isMountedMainCtx) {
                    // إعادة التحقق من حالة الاتصال
                    final isDeviceSecure = await SecurityIntegration.instance
                    ...
                    if (isMountedMainCtx) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            isDeviceSecure
                                ? 'البيئة آمنة. جميع خدمات الأمان تعمل بشكل صحيح.'
                                : 'تحذير: تم اكتشاف مشكلة في خدمات الأمان.',
                            style: GoogleFonts.cairo(),
                          ),
                          backgroundColor:
                              isDeviceSecure ? Colors.green : Colors.orange,
                        ),
                      );
                    }
                  }
              ...
              _buildBottomSheetTile(
                ...
                onTap: () async {
                  ...
                  if (isMountedMainCtx) {
                    ...
                    logger.warn("SelfDestructMenuOption",
